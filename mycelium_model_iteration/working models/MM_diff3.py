...
# ---- Hypha growth ----
MU_MAX = 2.48            # maximum tip growth rate (µm/hr)
...
#      spread used at initialisation — i.e. the same `init_gaussian_field`
#      machinery (Gaussian, clip to ±pm, exact-mean recentre).
#   3. Write the new field into liquid voxels only. Hypha voxels stay at 0
#      (perfect sink) and solid voxels stay NaN.
#
# Physical interpretation: this is the limit of "let the liquid sit static long
# enough to re-equilibrate" — no diffusion coefficients, no sub-steps, no
# species-relative timescales. Useful as a comparison point for the PDE model.

# Module-level RNG so successive calls don't repeat the same draw pattern but
# the run is still reproducible from RNG_SEED.
_NUTRIENT_AVG_RNG = np.random.default_rng(RNG_SEED + 1)


def average_nutrients(grid: VoxelGrid3D):
    """Redistribute each nutrient as a fresh Gaussian field at the post-consumption mean.

    Mirrors `initialize_nutrients`: same sigma = pm · SIGMA_SCALE[name], same
    clip rules, same exact-mean recentre — only the target mean is the current
    liquid average instead of NUTRIENT_SPECS[name]["mean"].
    """
    ...
    for name, spec in NUTRIENT_SPECS.items():
        ...
        # Step 1: new mean = current liquid average for this species.
        ...
        # Step 2: re-sample the field at the new mean (same procedure as init).
        pm = float(spec["pm"])
        sigma = pm * float(SIGMA_SCALE[name])
        clip = pm if CLAMP_TO_PM_RANGE else None
        ...
        # Step 3: write to liquid voxels only; preserve hypha=0, solid=NaN.
        out = arr.copy()
...
if __name__ == "__main__":
    ...
    for step in range(N_STEPS):
        ...
        # 5. Nutrient averaging (well-mixed bath; re-sample at post-consumption mean)
