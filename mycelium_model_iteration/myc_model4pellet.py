...
# ---- Grid / domain (µm) ----
EXTENT_UM = 200.0
CONIDIA_SPAWN_BOX_EXTENT_UM = 40.0
N_GRID = 65
RNG_SEED = 8
...
def _radial_curve_with_inner_avg(ax, distances, values, color, label, inner_cutoff_um):
    ...
    if np.any(mask_outer):
        ...
        ax.plot(d_plot, v_plot, 'o-', linewidth=2, markersize=6,
                color=color, label=label)
    elif avg_inner is not None:
        ax.plot([], [], 'o-', linewidth=2, markersize=6, color=color, label=label)
...
def plot_nutrient_depletion_analysis(nutrient_data_list, timesteps_list, nutrient_name):
    ...
    ax.set_xlabel('Distance from center (µm)', fontsize=12)
    ax.set_ylabel(f'{nutrient_name} depletion (mM)', fontsize=12)
...
def plot_citric_acid_production_analysis(citric_data_list, timesteps_list):
    ...
    ax.set_xlabel('Distance from center (µm)', fontsize=12)
    ax.set_ylabel('Citric acid production (mmol)', fontsize=12)
