...
import matplotlib
matplotlib.use("Agg")  # non-interactive backend: never opens windows during sensitivity sweeps.
...
# ---- Apical branching ----
PSI_BRANCH = 0.97             # branch when  ψ · μ_{t-1} ≥ μ_t  (drop-in-rate threshold)
...
# Snapshot of baseline tunables — used by run_simulation() to reset module state
# between sweep runs so each call starts from the same "current code" values.
_DEFAULTS = {
    "RNG_SEED":                  RNG_SEED,
    "MU_MAX":                    MU_MAX,
    "PSI_BRANCH":                PSI_BRANCH,
    "HYPHA_RADIUS_UM":           HYPHA_RADIUS_UM,
    "LATERAL_DISTANCE_FROM_TIP": LATERAL_DISTANCE_FROM_TIP,
    "PHI_N":                     PHI_N,
    "PHI_P":                     PHI_P,
    "NUTRIENT_SPECS":            copy.deepcopy(NUTRIENT_SPECS),
}


def _apply_overrides(overrides):
    """Reset tunable globals to baseline, then apply `overrides`.

    Supported override keys:
      - RNG_SEED, MU_MAX, PSI_BRANCH, HYPHA_RADIUS_UM,
        LATERAL_DISTANCE_FROM_TIP, PHI_N, PHI_P  (scalar overrides)
      - NUTRIENT_PM_FRACTION:  float — rescales pm to (fraction * mean) for ALL nutrients.
    """
    global RNG_SEED, MU_MAX, PSI_BRANCH, HYPHA_RADIUS_UM
    global LATERAL_DISTANCE_FROM_TIP, PHI_N, PHI_P, NUTRIENT_SPECS

    # Restore baseline first (so we never compound effects across runs).
    RNG_SEED                  = _DEFAULTS["RNG_SEED"]
    MU_MAX                    = _DEFAULTS["MU_MAX"]
    PSI_BRANCH                = _DEFAULTS["PSI_BRANCH"]
    HYPHA_RADIUS_UM           = _DEFAULTS["HYPHA_RADIUS_UM"]
    LATERAL_DISTANCE_FROM_TIP = _DEFAULTS["LATERAL_DISTANCE_FROM_TIP"]
    PHI_N                     = _DEFAULTS["PHI_N"]
    PHI_P                     = _DEFAULTS["PHI_P"]
    NUTRIENT_SPECS            = copy.deepcopy(_DEFAULTS["NUTRIENT_SPECS"])

    if not overrides:
        return

    for key, value in overrides.items():
        if key == "NUTRIENT_PM_FRACTION":
            for nutr in NUTRIENT_SPECS:
                NUTRIENT_SPECS[nutr]["pm"] = float(NUTRIENT_SPECS[nutr]["mean"]) * float(value)
        elif key == "RNG_SEED":
            RNG_SEED = int(value)
        elif key == "MU_MAX":
            MU_MAX = float(value)
        elif key == "PSI_BRANCH":
            PSI_BRANCH = float(value)
        elif key == "HYPHA_RADIUS_UM":
            HYPHA_RADIUS_UM = float(value)
        elif key == "LATERAL_DISTANCE_FROM_TIP":
            LATERAL_DISTANCE_FROM_TIP = float(value)
        elif key == "PHI_N":
            PHI_N = float(value)
        elif key == "PHI_P":
            PHI_P = float(value)
...
def run_simulation(overrides: dict = None, verbose: bool = False) -> dict:
    """Run one MDM simulation (no plots, no CSVs). Returns end-of-run metrics.

    Pass `overrides` to vary baseline tunables. See `_apply_overrides`.
    """
    ...
    # If not all conidia could be placed (e.g. crowding for a particular RNG seed),
    # bail out so the sensitivity driver can skip this data point cleanly.
    if len(conidia) < N_CONIDIA:
        if verbose:
            print(f"  [SKIP] only {len(conidia)}/{N_CONIDIA} conidia placed — discarding this run.")
        return None
    for c in conidia:
    ...
    m = None  # latest compute_metrics result; populated inside the loop
    ...
    for step in range(N_STEPS):
        ...
        if verbose:
            print(
                ...
                f"tips={m['total_tips']}  branches={m['total_branch_nodes']}  "
                f"len={m['total_length_um']:.1f}µm  CA(cum)={m['citric_total_mmol']:.4g} mmol"
            )

    return {
        "total_tips":         int(m["total_tips"]),
        "n_apical_events":    int(branch_event_counters["apical"]),
        "n_lateral_events":   int(branch_event_counters["lateral"]),
        "avg_length_um":      float(m["avg_length_um"]),
        "avg_volume_um3":     float(m["avg_volume_um3"]),
        "citric_total_mmol":  float(m["citric_total_mmol"]),
    }


if __name__ == "__main__":
    # Allow running the module standalone for a single baseline check.
    print("Running single baseline simulation (no overrides)...")
    metrics = run_simulation(verbose=True)
    print("\nFinal metrics:")
