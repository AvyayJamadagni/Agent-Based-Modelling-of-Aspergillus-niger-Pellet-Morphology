...
SAVE_FIGURES = True   # save figures as PNGs to FIGURES_DIR instead of opening them interactively
FIGURE_DPI = 150

# ---- Grid / domain (µm) ----
EXTENT_UM = 150
CONIDIA_SPAWN_BOX_EXTENT_UM = 50.0
N_GRID = 90
RNG_SEED = 4
...
# ---- Multiprocessing ----
# Number of worker processes used for the parallel consume_nutrients pass.
# Set to 1 to disable parallelism entirely. Default = (CPU count - 1) so the OS
# stays responsive. Tip elongation, lateral branching, and diffusion stay serial
# (they read/write shared graph + grid state and are not amenable to safe parallelism).
N_WORKERS = max(1, (os.cpu_count() or 2) - 1)

# ---- Nutrients (mM) ----
NUTRIENT_SPECS = {
    "Nitrogen":   {"mean": 70.0,  "pm": 3.5},   
    "Phosphorus": {"mean": 0.011,   "pm": 0.0005},    
    "Glucose":    {"mean": 55.5,  "pm": 2.775},     
...
# ---- Hypha growth ----
MU_MAX = 1.77            # maximum tip growth rate (µm/hr)
DEPLETION_RADIUS = 5.0  # radius (µm) around a tip used for sensing & depletion
HYPHA_RADIUS_UM = 1.8   # physical radius of the hypha cylinder (µm)

# ---- Apical branching ----
PSI_BRANCH = 0.8             # branch when  ψ · μ_{t-1} ≥ μ_t  (drop-in-rate threshold)
MIN_BRANCH_ANGLE_DEG = 50.0   # minimum angle (deg) between the two daughter directions
APICAL_BRANCH_MIN_STEP = 10    # apical branching disabled while step_idx < this

# ---- Lateral branching ----
LATERAL_DISTANCE_FROM_TIP = 12.7   # min distance (µm) from any tip for a lateral branching site
PHI_N = 0.36                      # local N ≥ PHI_N · [N]_init required for a lateral branch
PHI_P = 0.36                      # local P ≥ PHI_P · [P]_init required for a lateral branch
LATERAL_BRANCH_MIN_STEP = 10       # lateral branching disabled while step_idx < this
...
DIFFUSION_COEFFS_M2_PER_S = {
    "Nitrogen":   1.7e-9,
    "Phosphorus": 8.79e-10,    # phosphate
    "Glucose":    6.0e-10,
    "Oxygen":     2.12e-9,
}
DIFFUSION_STRENGTH = 0.15       # α for the fastest species (must be ≤ 1/6 for stability in 3D)
DIFFUSION_SUBSTEPS = 70         # number of explicit-Euler sub-iterations per simulation step
...
def _save_or_show(filename: str):
    ...
    If SAVE_FIGURES is False, show the figure interactively instead. Either way,
    the figure is closed so the next plot starts fresh.
    """
    if SAVE_FIGURES:
...
class VoxelGrid3D:
    def __init__(self, extent_um=50.0, n=24, origin_um=(0.0, 0.0, 0.0)):
        ...
        # Pre-compute and cache the center meshgrid (used heavily by every sphere op).
        # This is fixed for the entire simulation since the grid coordinates never change,
        # so we build it once here instead of on every call.
    ...
    def rebuild_center_mesh(self):
        # Returns the cached meshgrid built in __init__. Constant for the simulation lifetime.
...
def grow_tip(grid: VoxelGrid3D, G: nx.Graph, cid: int, tip_node: int, step_idx: int, _means: dict = None):
    ...
    If the top-1 direction's μ-step lands in solid/hypha/out-of-bounds, we walk down
    the score-ranked, cone-filtered candidate list and try the next-best direction,
    until one yields a valid landing voxel. This rescues tips that would otherwise
    silently fail (common when μ < dx and the new position rounds back into the
    tip's own voxel).
    """
    ...
    # Walk through cone-filtered candidates in score order until one lands in a liquid voxel.
...
# Strategy
# --------
# `consume_nutrients` is the dominant per-step cost. We parallelize it by:
...
#      worker processes can read them without pickling 5 MB per task.
#   3. Each worker computes the SPARSE delta its task applies (just the voxel
#      indices and the per-voxel Δ values), and returns it.
#   4. The main process merges all sparse deltas into the grid serially.
#
# Semantic note
# -------------
# In the serial path, each consumer sees the depletions of consumers processed
# before it within the same step. In the parallel path, every consumer sees the
# START-of-pass state (the snapshot pushed into shared memory). Their deltas
# are then merged — overlapping spheres can collectively over-deplete past 0,
# but we clamp at 0 when applying. For our scale this is a fine approximation,
# and it's the only way to expose meaningful task-level parallelism in Python
# without locks on every voxel write.

# ---- Worker process globals ----
# These are populated by `_consume_worker_init` once per worker, and they read
# from shared memory written by the main process at the start of each
# parallel consume call.
_W_NUTRIENTS = None         # dict[name -> np.ndarray view into shared memory]
_W_CITRIC = None            # np.ndarray view (citric_acid_spatial_mM)
_W_HYPHA_MASK = None        # np.ndarray view (bool)
_W_SOLID_MASK = None        # np.ndarray view (bool)
_W_XC = None                # cached meshgrid (built locally in worker)
_W_YC = None
_W_ZC = None
_W_VOXEL_VOL_L = None
...
_W_K_G2 = None
_W_K_I2 = None
_W_LAMBDA_CA = None         # CITRIC_mmol_per_mol_GLUCOSE
...
def _consume_worker_init(metadata):
    ...
    # Build local meshgrid (small enough to duplicate per worker; avoids pickling).
...
def _consume_worker_compute(task):
    ...
    Return: (cid, voxel_idx (k,3), nut_deltas_mM dict, citric_mmol, citric_delta_mM)
            or None if no liquid voxels in sphere.
    """
    cid, point_um, biomass_g, rate_factor = task

    # Sphere mask (uses worker-local cached meshgrid)
    dx = _W_XC - point_um[0]
    dy = _W_YC - point_um[1]
    dz = _W_ZC - point_um[2]
    ...
    # Means over liquid in sphere (per nutrient)
    means = {}
    ...
    # Citric production from active pathway
    citric_mmol = 0.0
    citric_delta_mM = 0.0
    if v_G_active > 0.0:
        active_glucose_mmol = v_G_active * biomass_g * float(rate_factor)
...
class _SharedGridState:
    """Manages shared-memory mirrors of the dynamic grid arrays the workers need.

    Lifetime:
      - Created once by the main process before the simulation loop starts.
      - `sync_from(grid)` is called at the start of each parallel consume pass
        to copy the current state in.
    ...
    def __init__(self, grid: "VoxelGrid3D"):
        ...
        # Helper: create a SharedMemory + ndarray view, copy initial data in.
        def _make(name, src):
...
def _build_consumption_tasks(graphs):
    """Flatten tips and edges into a list of consumption tasks."""
    tip_biomass = cylinder_biomass_g(DEPLETION_RADIUS, HYPHA_RADIUS_UM)
    tasks = []
...
def consume_nutrients_parallel(grid: VoxelGrid3D, graphs: dict, pool, shared_state: _SharedGridState):
    ...
    if pool is None:
        # No pool — run serially via the same worker function (for debugging parity).
    ...
    # Distribute. Pool processes count is set at construction; we don't need
    # to chunk manually — Pool's default chunksize works fine for ~hundreds of tasks.
...
def diffuse_nutrients(grid: VoxelGrid3D):
    """Explicit-Euler 6-neighbour diffusion with DIFFUSION_SUBSTEPS inner iterations.

    Each call applies N = DIFFUSION_SUBSTEPS stable explicit-Euler updates per
    nutrient species, giving diffusion finer effective time resolution than the
    rest of the simulation step. Total mixing per simulation step scales linearly
    with DIFFUSION_SUBSTEPS, so bump that constant for more equilibration.
    """
    ...
    # Neighbour-count tensor depends only on the liquid mask, not on any nutrient
    # field, so we compute it once outside the loops below.
    cnt = liquid.astype(np.float64)
    ...
    n_substeps = max(1, int(DIFFUSION_SUBSTEPS))
    ...
    for nutr, arr in grid.nutrients.items():
        ...
        valid = liquid & np.isfinite(arr)
        cur = arr.astype(np.float64, copy=True)

        # ---- Inner Euler sub-steps ----
        for _ in range(n_substeps):
            arr_m = np.where(valid, cur, 0.0)
            ...
            updated = np.maximum(0.0, arr_m + delta)
            # Apply only to valid (liquid) voxels; np.where preserves the original
            # values at hypha (=0, perfect sink) and solid (=NaN) cells. Mass that
            # diffuses INTO a hypha voxel during this sub-step is therefore consumed.
            cur = np.where(valid, updated, cur)

        new_arr = cur.astype(np.float32)
...
# =============================================================================
#
# Pellet center is the centroid of the conidia (computed in __main__). Space is
# divided into N_SHELLS concentric spherical shells out to the furthest hypha
# node from the FINAL graph state — every snapshot reuses the same bins for fair
# comparison.
#
# Three per-snapshot analyses are run:
...
#
# Plus a 2D cross-section visualization with all shells (and an optional
# highlighted shell drawing the edges that fall inside it in red).
...
def analyze_citric_acid_by_shell(center_um, grid, citric_spatial_before,
    ...
    Model 4 stores citric acid as a per-voxel CONCENTRATION (mM), so we sum
    Δ[CA] across voxels in the shell and multiply by voxel_volume_L to get mmol.
    """
    n_shells = len(shell_radii) - 1
    ...
    for i in range(n_shells):
        ...
        in_shell = (distances >= r_inner) & (distances < r_outer)
        if np.any(in_shell):
            total_mmol = float(np.sum(citric_delta_mM[in_shell]) * voxel_vol_L)
        else:
            total_mmol = 0.0
...
def plot_shell_cross_section(center_um, graphs, shell_data, highlight_shell_idx=None):
    """XY-plane cross-section. Concentric circles for shells, hypha edges in gray.
    Optionally highlights one shell (its boundary thicker, edges within it drawn red).
    """
...
def _radial_curve_with_inner_avg(ax, distances, values, color, label, inner_cutoff_um):
    """Plot a radial curve with the inner shells averaged into a single horizontal line.

    Shared helper for hyphal-fraction and nutrient-depletion plots — collapses
    shells inside `inner_cutoff_um` to a single mean (smooths conidium-placement
    noise near the center) and shows individual points only for outer shells.
    """
    ...
    if avg_inner is not None:
        ax.plot([0, inner_cutoff_um], [avg_inner, avg_inner],
    ...
    if np.any(mask_outer):
        ...
        ax.plot(d_plot, v_plot, 'o-', linewidth=3, markersize=7,
...
def plot_hyphal_density_analysis(shell_data_list, timesteps_list):
    """Hyphal volume fraction vs distance from center, one viridis-colored curve per snapshot."""
    ...
    for i, (shell_data, step) in enumerate(zip(shell_data_list, timesteps_list)):
        ...
        _radial_curve_with_inner_avg(
            ax,
            shell_data['shell_mid'],
            shell_data['hyphal_fractions'],
            colors[i],
            f'Step {step} ({time_hr:.1f} hr)',
            inner_cutoff,
        )
...
def plot_nutrient_depletion_analysis(nutrient_data_list, timesteps_list, nutrient_name):
    """Nutrient depletion (mM) vs distance from center, one viridis-colored curve per snapshot."""
    fig, ax = plt.subplots(figsize=(12, 7))
    ...
    for i, (nutrient_data, step) in enumerate(zip(nutrient_data_list, timesteps_list)):
        ...
        _radial_curve_with_inner_avg(
            ax,
            nutrient_data['shell_mid'],
            nutrient_data['nutrient_delta_avg'],
            colors[i],
            f'Step {step} ({time_hr:.1f} hr)',
            inner_cutoff,
        )
...
def plot_citric_acid_production_analysis(citric_data_list, timesteps_list):
    """Citric acid production per shell (mmol) vs distance from center."""
    fig, ax = plt.subplots(figsize=(12, 7))
...
def export_shell_data_csv(shell_data):
    """Export final-timestep hyphal-density shell breakdown to CSV."""
    rows = []
...
def export_hyphal_density_multistep_csv(shell_data_list, timesteps_list):
    """Per-shell hyphal density data across ALL snapshot timesteps (one row per (step, shell))."""
    rows = []
...
def export_nutrient_depletion_multistep_csv(nutrient_data_list, timesteps_list, nutrient_name):
    """Per-shell nutrient depletion (mM) across ALL snapshot timesteps for one species."""
    rows = []
    ...
    write_csv(
        ...
        fieldnames=['step', 'time_hr', 'shell_index', 'r_inner_um', 'r_outer_um',
                    'r_mid_um', 'nutrient_before_mM', 'nutrient_after_mM',
                    'nutrient_delta_mM'],
        rows=rows,
    )
...
def export_citric_production_multistep_csv(citric_data_list, timesteps_list):
    """Per-shell citric acid production (mmol) across ALL snapshot timesteps."""
    rows = []
...
if __name__ == "__main__":
    ...
    # Clamped to N_STEPS-1 to fix the off-by-one in Model 3 where the last entry could
    # equal N_STEPS itself and never fire (loop only reaches N_STEPS-1).
    ...
    # ---- Multiprocessing setup (skipped if N_WORKERS == 1) ----
    ...
    for step in range(N_STEPS):
        ...
        # 3a. Snapshot N/P/Glucose + citric BEFORE consumption (for radial analysis)
        ...
        if step in snapshot_timesteps:
            ...
            citric_snapshots[step]["after"] = copy.deepcopy(grid.metabolites["citric_acid_spatial_mM"])
        ...
        # 6. Save the post-step graph state for radial-density analysis
        if step in snapshot_timesteps:
    ...
    # Pellet center: centroid of all conidia (more biologically meaningful for a pellet
    # than the cube origin, which Model 3 used).
    pellet_center = np.mean(np.asarray(conidia, dtype=float), axis=0) if len(conidia) > 0 else grid.origin_um

    # Compute shell radii once from the FINAL graphs so every snapshot uses identical bins.
    ...
    for nutrient_name in ("Nitrogen", "Phosphorus", "Glucose"):
        ...
        for step in timesteps_list:
            ...
            nutrient_data_list.append(
                analyze_nutrient_by_shell(pellet_center, grid, before, after, nutrient_name, shell_radii)
            )
