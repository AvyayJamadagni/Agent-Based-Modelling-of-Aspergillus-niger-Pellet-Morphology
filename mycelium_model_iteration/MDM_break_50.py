...
# ---- Apical branching ----
PSI_BRANCH = 0.9931             # branch when  ψ · μ_{t-1} ≥ μ_t  (drop-in-rate threshold)
MIN_BRANCH_ANGLE_DEG = 50.0   # minimum angle (deg) between the two daughter directions
APICAL_BRANCH_MIN_STEP = 2    # apical branching disabled while step_idx < this

# ---- Lateral branching ----
LATERAL_DISTANCE_FROM_TIP = 4.6   # min distance (µm) from any tip for a lateral branching site
PHI_N = 0.9882                      # local N ≥ PHI_N · [N]_init required for a lateral branch
PHI_P = 0.9882                     # local P ≥ PHI_P · [P]_init required for a lateral branch
...
if __name__ == "__main__":
    ...
    # ---- Resolve the wall-clock deadline (absolute Unix time) ----
    # Priority: SIM_DEADLINE_EPOCH (absolute) > SIM_WALLTIME_SEC (relative budget).
    _deadline_epoch = None
    if os.environ.get("SIM_DEADLINE_EPOCH"):
        _deadline_epoch = float(os.environ["SIM_DEADLINE_EPOCH"])
    elif os.environ.get("SIM_WALLTIME_SEC"):
        _deadline_epoch = time.time() + float(os.environ["SIM_WALLTIME_SEC"]) - SIM_SAFETY_SEC

    if _deadline_epoch is not None:
        _remaining = _deadline_epoch - time.time()
        print(f"[deadline] Wall-clock stop set for {time.ctime(_deadline_epoch)} "
              f"({_remaining/3600.0:.2f} h from now). Post-processing runs after the stop.")
    else:
        print("[deadline] No wall-clock deadline set — running all N_STEPS.")

    interrupted = False

    # ---- Main loop: branching → consumption → metrics → averaging ----
    for step in range(N_STEPS):
        # 0. Wall-clock check. If the deadline has passed, stop the simulation
        #    at this step but still run it fully AND force a snapshot so the
        #    radial analyses include the latest state, then break afterwards.
        if _deadline_epoch is not None and time.time() >= _deadline_epoch:
            interrupted = True
            print(f"[deadline] Wall-clock limit reached at step {step}. "
                  f"Running this final step, then processing results so far.")
            if step not in snapshot_timesteps:
                snapshot_timesteps = sorted(set(snapshot_timesteps) | {step})
    ...
    # Number of steps actually completed (may be < N_STEPS if interrupted).
    completed_steps = len(history["total_length_um"])
    if interrupted:
        print(f"\n[deadline] Simulation stopped early after {completed_steps}/{N_STEPS} steps. "
              f"Processing and saving all results gathered so far...")

    plot_skeletons_3d(graphs, grid, title=f"Hypha skeletons after {completed_steps} steps")
